...
export const imageApi = {
  ...
  getImagesByStatus: async (status: string): Promise<Image[]> => {
    const response: ImagesResponse = await apiRequest(`/images/status/${status}`);
    if (!response.success) {
      throw new Error(response.error || 'Failed to fetch images');
    }
    return response.data || [];
  ...
  // Upload single image
  uploadImage: async (file: File, title: string, tags: string[], uploadedBy: string): Promise<Image> => {
...
export const adminApi = {
  ...
  approveImage: async (id: number, approvedBy: string): Promise<Image> => {
    ...
    const response: ImageResponse = await apiRequest(`/admin/images/${id}/approve`, {
      method: 'POST',
  ...
  // Reject image
  rejectImage: async (id: number, rejectedBy: string, rejectionReason?: string): Promise<Image> => {
    const request: RejectImageRequest = { rejectedBy, rejectionReason };
    const response: ImageResponse = await apiRequest(`/admin/images/${id}/reject`, {
      method: 'POST',
      body: JSON.stringify(request),
    });
    if (!response.success) {
